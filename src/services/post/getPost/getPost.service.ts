 ...
 * publishedAt 기준 인접(이전/다음) 글을 함께 반환한다.
 */
export async function getPost(slug: string): Promise<GetPostResult> {
  ...
  const [prev, next] = post.publishedAt
    ? await Promise.all([
        PostRepository.getAdjacent({ publishedAt: post.publishedAt, direction: 'prev' }),
        PostRepository.getAdjacent({ publishedAt: post.publishedAt, direction: 'next' }),
      ])
    : [null, null];
